...
func (c Exec) SubCommands() []util.SubCommand {
	return []util.SubCommand{
		util.NewSubCommand("^jarvis exec sh (?P<command>.+)$", c.Sh),
		util.NewSubCommand("^jarvis exec python (?P<command>.+)$", c.Python),
	}
}

func (c Exec) Sh(m util.IncomingSlackMessage, r util.Regex) {
	command := r.SubExpression(m.Text, 0)
	result, err := service.Docker{}.RunShInContainer(command, 10*time.Second)
	if len(result) > 0 {
